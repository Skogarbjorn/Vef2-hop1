...
import xss from 'xss';
import { identifierValidator } from '../validate/validators.js';

const { BCRYPT_ROUNDS: bcryptRounds = 1 } = process.env;

export async function createUser(username, email, password) {
	const password_hashed = await bcrypt.hash(password, parseInt(bcryptRounds, 10));
...
export async function verifyPassword(user, password) {
	console.info("verifying password for user", user);
	console.info("with password", password);
